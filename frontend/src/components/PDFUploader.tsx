...
export default function PDFUploader({ onUploaded }: PDFUploaderProps) {
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDropRejected: (rejections) => {
      ...
      toast.error('File tidak valid. Pastikan format PDF dan ukuran file sesuai.');
    },
  ...
  const handleUpload = async () => {
    ...
    const nextUploadId = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    setUploadId(nextUploadId);

    try {
      const formData = new FormData();
      formData.append('file', file);
      formData.append('upload_id', nextUploadId);
  ...
  return (
    <div className="space-y-3">
      ...
      >
        ...
        <p className="text-xs text-gray-400 mt-1">Maks. 20MB - Hanya file PDF</p>
      </div>
